...
/// `extras-demo ignore` evaluates paths against one or more combined ignore files.
///
/// Loads every `--file` with `IgnoreProcessor` and combines them
/// left-to-right with `+`: a later `--file` overrides an earlier one
/// wherever both match, mirroring git's own layered ignore sources (e.g.
/// `.git/info/exclude` then `.gitignore`).
///
...
struct IgnoreCommand: AsyncParsableCommand {
    ...
    /// An ignore file to load.
    ///
    /// May be repeated; files combine left-to-right with `+`, so a later
    ...
    ///
    /// A trailing slash probes the path as a directory (git's
    /// `check-ignore` convention).
    @Argument(
