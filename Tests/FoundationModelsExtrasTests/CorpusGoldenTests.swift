...
@Suite struct CorpusGoldenTests {
    ...
    private static let expectedPopulatedRoot = fixturesRoot.appendingPathComponent("Expected", isDirectory: true)

    /// The checked-in expected output mirror for `absentContext()` — pins
    /// every `{% if %}` site's false branch.
    private static let expectedAbsentRoot = fixturesRoot.appendingPathComponent("ExpectedAbsent", isDirectory: true)
    ...
    private static let regenerateGoldens = ProcessInfo.processInfo.environment["CORPUS_GOLDEN_UPDATE"] == "1"
    ...
    private static func makeEngine() -> TemplateEngine {
        ...
        return TemplateEngine(partials: stack, environment: [:], wellKnownValues: fixtureWellKnownValues)
    }
    ...
    private static func diffDescription(expected: String, actual: String) -> String {
        ...
        return "no line-level difference found (\(expectedLines.count) vs \(actualLines.count) lines — trailing newline?)"
    }
    ...
    @Test func everyCorpusFileRendersAndMatchesItsExpectedOutputUnderThePopulatedContext() throws {
        try Self.assertCorpusMatchesExpected(
            contextLabel: "populated", context: Self.populatedContext(), expectedRoot: Self.expectedPopulatedRoot)
    }
    ...
    @Test func migratedConstructsNoLongerAppearInTheCorpusAndRenderCorrectly() throws {
        ...
        let skills = try String(contentsOf: Self.corpusRoot.appendingPathComponent(skillsPath), encoding: .utf8)
        #expect(!skills.contains(".size"))
        #expect(skills.contains("available_skills.count > 0"))
        let renderedWithSkills = try engine.render(skills, context: Self.populatedContext(), trust: .trusted)
        let renderedWithoutSkills = try engine.render(skills, context: Self.absentContext(), trust: .trusted)
    ...
    /// the way it does.
    @Test func noSecretsPlaceholderLiteralIsSilentlyEatenByStencilAndTheOutputIsDeliberatelyPinned() throws {
        let engine = Self.makeEngine()
