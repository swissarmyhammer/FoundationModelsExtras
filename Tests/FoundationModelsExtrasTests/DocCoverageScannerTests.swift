...
///
/// Mirrors the family's doc-coverage convention (see
/// `FoundationModelsShelltool`'s `DocCoverageScannerTests.swift`).
...
struct DocCoverageScannerTests {
  ...
  func undocumentedPublicStructIsAViolation() {
    ...
    #expect(
      violations == [DocCoverageScanner.Violation(filePath: "Fixture.swift", line: 1, name: "Foo")])
  }
  ...
  func docCommentSeparatedByBlankLineIsNotAttached() {
    ...
    let violations = DocCoverageScanner.violations(in: source, filePath: "Fixture.swift")
    #expect(
      violations == [DocCoverageScanner.Violation(filePath: "Fixture.swift", line: 3, name: "Foo")])
  }
  ...
  @Test(
    "an undocumented case in a public enum is a violation despite carrying no modifier of its own")
  func undocumentedCaseInAPublicEnumIsAViolation() {
    ...
    let violations = DocCoverageScanner.violations(in: source, filePath: "Fixture.swift")
    #expect(
      violations == [DocCoverageScanner.Violation(filePath: "Fixture.swift", line: 3, name: "bar")])
  }
