...
/// Enforces the family's "DocC comments on all public API" convention: every
/// `public` declaration anywhere under `Sources/FoundationModelsExtras` must
/// have a `///` doc comment directly attached to it (per the codebase's own
/// established convention — see e.g. `Sources/FoundationModelsExtras/DotfolderStack.swift`).
///
/// This is family convention, not plan.md scope — kept because every sibling
/// package enforces it (see `FoundationModelsShelltool`'s `DocCoverageTests`).
///
...
/// integration check against the real source tree.
@Suite("Public API doc coverage")
struct DocCoverageTests {
    @Test("every public declaration in Sources/FoundationModelsExtras has an attached doc comment")
    func foundationModelsExtrasSourceIsFullyDocumented() throws {
        let violations = try DocCoverageScanner.scan(directory: "Sources/FoundationModelsExtras")
...
/// declaration (or its leading attributes, e.g. `@available`) by exactly one
/// newline — no blank line in between — matching how this codebase's existing,
/// review-enforced doc comments are written throughout `Sources/FoundationModelsExtras`.
enum DocCoverageScanner {
    ...
    /// Scans every `.swift` file at or below `directory` (recursively) for
