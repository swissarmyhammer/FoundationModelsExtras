...
@Suite struct UntrustedRenderingTests {
    ...
    struct Fixture {
        ...
        func writePartial(_ contents: String, named name: String) {
            let fileURL = projectDirectory.appendingPathComponent("_partials").appendingPathComponent(name)
    ...
    @Test func hugeForLoopOverAContextProvidedCollectionTerminatesWithADescriptiveErrorInsteadOfHanging() {
        let engine = Fixture().makeEngine()
